...
void node_init(int argc, char *argv[]) {
    ...
    map_add(node_map, "node.config.logValuesFileClearSeconds", &node.config.logValuesFileClearSeconds, TYPE_UINT16);
    map_add(node_map, "node.config.logMilliseconds", &node.config.logMilliseconds, TYPE_UINT16);
    map_add(node_map, "node.config.tmpfs.use", &node.config.tmpfs.use, TYPE_BOOL);
    map_add(node_map, "node.config.tmpfs.sizeMB", &node.config.tmpfs.sizeMB, TYPE_UINT16);
    ...
    map_add(node_map, "node.config.influxDB.connectTimeoutMs", &node.config.influxDB.connectTimeoutMs, TYPE_UINT16);
    ...
    map_add(node_map, "node.config.network.mqPublish", &node.config.network.mqPublish, TYPE_BOOL);
    map_add(node_map, "node.config.network.mqBrokermDNSserviceType", &node.config.network.mqBrokermDNSserviceType, TYPE_STRING);
    map_add(node_map, "node.config.network.httpPort", &node.config.network.httpPort, TYPE_UINT16);
